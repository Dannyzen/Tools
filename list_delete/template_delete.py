...
response = sailthru_client.api_get('template', data)
jdata = json.loads(response)
print jdata

#[ (the thing I want) for (variable to iterate) in (some other list)]
rmlist=([l['name'] for l in jdata['templates']]) 
for list_name in rmlist:
	todelete={"template":list_name}
	response = sailthru_client.api_delete('template', todelete)
	print response
